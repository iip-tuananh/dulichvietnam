      ...
      if (this.errors.length > 0) {
        ...
        this.saveProduct(this.objData)
          .then(response => {
            ...
            this.$success('Sửa tour thành công');
            // this.$route.push({name:'listProduct'});
          })
          .catch(error => {
            this.loadings(false);
            this.$error('Sửa tour thất bại');
          });
      }
    },
    editById() {
